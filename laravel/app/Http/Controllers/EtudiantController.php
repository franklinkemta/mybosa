    ...
    {   
        ...
        switch ($section_id) {
            ...
            case 4: { // Section Documents
                ...
                // check if the documents etudiants section has been initialized first
                if (!$documentsEtudiant) {
                    Log::info('Etudiant '.$etudiant->id.': Initialisation de la documents');
                    $documentsEtudiant = DocumentsEtudiant::create(['etudiant_id' => $etudiant->id]);
                }
                return view('etudiant.dossierCandidat')->with(['section_id' => $section_id, 'documentsEtudiant' => $documentsEtudiant]);
            }
            break;
            case 5: { // Last section : result
    ...
    {   
        ...
        if ($request->hasAny($sectionFields) && !$validator->fails()) {
            ...
            if ($request->hasFile('photo') && $request->file('photo')->isValid()) {
                ...
                $photo = Storage::putFile('photos_etudiants', $request->file('photo'));
            ...
            $documentsEtudiant->save();

            $etudiant->section4_remplie = $documentsEtudiant->wasChanged(); // Profil completion check
            $etudiant->save();
